...
Authors: David Fisher and Hao Hu.
...
def main():
    ...
    while time_s != 0:
        speed_motor_sp = int(input("Enter a speed (0 to 900 dps):"))
        distance = int(input("Enter a time to drive (seconds): "))
        left_motor.run_forever(speed_sp=speed_motor_sp)
        right_motor.run_forever(speed_sp=speed_motor_sp)
        time.sleep(distance/speed_motor_sp)
        left_motor.stop()
